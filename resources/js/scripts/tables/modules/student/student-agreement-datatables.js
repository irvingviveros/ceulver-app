...
$(function () {
    ...
    if (dt_basic_table.length) {
        var dt_basic = dt_basic_table.DataTable({
            ...
            serverSide: true,
            ajax: ({
                url: window.location.origin + '/api/agreements',
                type: 'GET',
            }),
                ...
                },
                {
                    ...
                    render: function (data, type, full, meta) {
                        ...
                        return (
                            ...
                            'Detalles</a>' +
                            '<a href="javascript:;"' + ' data-id="' + $agreement_id + '"' + ' class="dropdown-item delete-record">' +
                            feather.icons['trash-2'].toSvg({ class: 'font-small-4 me-50' }) +
                            'Eliminar</a>' +
                            '</div>' +
                            '</div>' +
                            '<a href="/admin/manage-students/agreement' + $agreement_id + '/edit"' + ' class="item-edit">' +
            ...
            responsive: {
                details: {
                    display: $.fn.dataTable.Responsive.display.modal({
                        header: function (row) {
                            var data = row.data();
                            return 'Detalles de ' + data['agreement'];
                        }
                    }),
                    type: 'column',
        ...
        $('div.head-label').html('<h6 class="mb-0">Administrar convenios</h6>');
    }
